...
// ---------------------------------------------------------------------------
// LLM Bridge (embedded Node.js script)
// ---------------------------------------------------------------------------
...
function collectProjectContext(pp) {
  const files = ['AGENTS.md', '.cursorrules', 'CLAUDE.md', '.nexus.md'];
  const parts = [];
  for (const f of files) {
    const fp = resolve(pp, f);
    if (existsSync(fp)) {
      try {
...
function emit(type, data) { process.stdout.write(JSON.stringify({ type, data }) + '\n'); }
function nexusDir() {
  const h = homedir();
...
async fn call_bridge_stream(
    ...
    let mut child = Command::new("node")
        .args(["--input-type=module", "-e", BRIDGE_SCRIPT, &args_str])
